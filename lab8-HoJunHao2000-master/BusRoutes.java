...
class BusRoutes {
   ...
   * @param services CompletableFuture of set of bus services between the two stops.
   */
   ...
   * @return CompletableFuture where the first line contains the query information:
  ...
  public CompletableFuture<String> description() {
    ...
    CompletableFuture<String> cs = CompletableFuture.supplyAsync(() -> result);

    for (BusService service : services.keySet()) {
      CompletableFuture<Set<BusStop>> stops = services.get(service);
      cs = cs.thenCombine(describeService(service, stops), (x, y) -> x + y);
    }
   ...
   * @param stops CompletableFuture of a set of bus stops served by the service
   * @return CompletableFuture of String where the first line contains the query information:
   *     bus stop id and search string.  The remaining line contains 
   *     the bus services and matching stops served.
   */
  public CompletableFuture<String> describeService(BusService service, 
      CompletableFuture<Set<BusStop>> stops) {
    CompletableFuture<String> cfs = stops.thenApply(
            sb -> {
              if (sb.isEmpty()) {
                return "";
              }

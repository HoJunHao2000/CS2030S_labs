...
class BusService {
   ...
   * @return CompletableFuture of set of bus stops that this bus services serves.
   */
  public CompletableFuture<Set<BusStop>> getBusStops() {
    CompletableFuture<Set<BusStop>> stops = BusAPI.getBusStopsServedBy(serviceId)
            .thenApply(s -> {
              ...
              Set<BusStop> sbs = sc.useDelimiter("\n")
                      .tokens()
                      .map(line -> line.split(","))
   ...
   * @return CompletableFuture of list of bus stops matching the given name.
   */
